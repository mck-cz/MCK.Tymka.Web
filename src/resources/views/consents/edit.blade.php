...
@section('title', __('messages.consents.edit_type'))

@section('content')
    <x-breadcrumb :items="[
        ['label' => __('messages.consents.title'), 'href' => route('consents.index')],
        ['label' => __('messages.consents.edit_type')],
    ]" />

    <h1 class="text-xl font-semibold mb-6">{{ __('messages.consents.edit_type') }}</h1>

    <form action="{{ route('consent-types.update', $consentType) }}" method="POST" class="card max-w-3xl">
        @csrf
        @method('PUT')
        <div class="card-body space-y-4">
            <div>
                <label for="name" class="form-label">{{ __('messages.consents.type_name') }} *</label>
                <input type="text" name="name" id="name" value="{{ old('name', $consentType->name) }}"
            ...
            <div>
                ...
                <input type="text" name="description" id="description" value="{{ old('description', $consentType->description) }}"
            ...
            <div>
                ...
                <div x-data="richEditor()" x-init="initEditor()">
                    <div x-ref="editor" class="bg-surface rounded-b-lg" style="min-height: 200px;">{!! old('content', $consentType->content ?? '') !!}</div>
                    <input type="hidden" name="content" x-ref="contentInput" value="{{ old('content', $consentType->content ?? '') }}">
                </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <input type="checkbox" name="is_required" id="is_required" value="1"
                    {{ old('is_required', $consentType->is_required) ? 'checked' : '' }} class="form-checkbox">
