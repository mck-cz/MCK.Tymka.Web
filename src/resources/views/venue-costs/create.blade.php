...
@section('content')
    ...
    <div class="card max-w-2xl">
        <div class="card-body">
            <form action="{{ route('venue-costs.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="cost_per_event" id="cost_per_event" value="{{ old('cost_per_event') }}" class="form-input w-full" min="0" step="1" required>
                    ...
                    <div>
                        ...
                        <select name="currency" id="currency" class="form-select w-full" required>
                            <option value="CZK" @selected(old('currency', 'CZK') === 'CZK')>CZK</option>
                ...
                <div>
                    ...
                    <select name="split_method" id="split_method" class="form-select w-full" required>
                        <option value="per_attendance" @selected(old('split_method') === 'per_attendance')>{{ __('messages.venue_costs.split_per_attendance') }}</option>
                        <option value="equal_monthly" @selected(old('split_method') === 'equal_monthly')>{{ __('messages.venue_costs.split_equal_monthly') }}</option>
                        <option value="fixed_per_member" @selected(old('split_method') === 'fixed_per_member')>{{ __('messages.venue_costs.split_fixed_per_member') }}</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="billing_period" id="billing_period" class="form-select w-full" required>
                        <option value="monthly" @selected(old('billing_period') === 'monthly')>{{ __('messages.venue_costs.period_monthly') }}</option>
                        <option value="seasonal" @selected(old('billing_period') === 'seasonal')>{{ __('messages.venue_costs.period_seasonal') }}</option>
                        <option value="per_event" @selected(old('billing_period') === 'per_event')>{{ __('messages.venue_costs.period_per_event') }}</option>
                    </select>
                        ...
                        @foreach(['training', 'match', 'competition', 'tournament'] as $type)
                            <label class="flex items-center gap-2 text-sm">
                                <input type="checkbox" name="include_event_types[]" value="{{ $type }}"
                                    class="form-checkbox"
                                    @checked(in_array($type, old('include_event_types', ['training'])))>
                ...
                <div>
                    ...
                    <input type="text" name="bank_account" id="bank_account" value="{{ old('bank_account') }}" class="form-input w-full">
