...
@section('title', __('messages.venues.edit'))

@section('content')
    <x-breadcrumb :items="[
        ['label' => __('messages.venues.title'), 'href' => route('venues.index')],
        ['label' => __('messages.venues.edit')],
    ]" />

    <h1 class="text-2xl font-semibold mb-6">{{ __('messages.venues.edit') }}</h1>

    <div class="card max-w-2xl">
        <div class="card-body">
            <form action="{{ route('venues.update', $venue) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="sport_type" id="sport_type" value="{{ old('sport_type', $venue->sport_type) }}"
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="form-input @error('notes') border-danger @enderror">{{ old('notes', $venue->notes) }}</textarea>
                    @error('notes')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_favorite" value="1" {{ old('is_favorite', $venue->is_favorite) ? 'checked' : '' }} class="form-checkbox">
                ...
                <div>
                    ...
                    <p class="text-xs text-muted mt-1" id="coords-display">
                        @if(old('latitude', $venue->latitude) && old('longitude', $venue->longitude))
                            {{ old('latitude', $venue->latitude) }}, {{ old('longitude', $venue->longitude) }}
                        @endif
                    </p>
                </div>
