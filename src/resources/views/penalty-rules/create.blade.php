...
@section('title', __('messages.penalties.create'))

@section('content')
    <x-breadcrumb :items="[
        ['label' => __('messages.penalties.title'), 'href' => route('penalty-rules.index')],
        ['label' => __('messages.penalties.create')],
    ]" />

    <h1 class="text-xl font-semibold mb-6">{{ __('messages.penalties.create') }}</h1>

    <div class="card max-w-2xl">
        <div class="card-body">
            <form action="{{ route('penalty-rules.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <select name="trigger_type" id="trigger_type" class="form-select w-full" required>
                        <option value="no_show" @selected(old('trigger_type') === 'no_show')>{{ __('messages.penalties.trigger_no_show') }}</option>
                        <option value="late_cancel" @selected(old('trigger_type') === 'late_cancel')>{{ __('messages.penalties.trigger_late_cancel') }}</option>
                        <option value="no_response" @selected(old('trigger_type') === 'no_response')>{{ __('messages.penalties.trigger_no_response') }}</option>
                    </select>
                ...
                <div>
                    ...
                    <select name="penalty_type" id="penalty_type" class="form-select w-full" required>
                        <option value="count_as_attended" @selected(old('penalty_type') === 'count_as_attended')>{{ __('messages.penalties.type_count_as_attended') }}</option>
                        <option value="fixed_amount" @selected(old('penalty_type') === 'fixed_amount')>{{ __('messages.penalties.type_fixed_amount') }}</option>
                        <option value="percentage_surcharge" @selected(old('penalty_type') === 'percentage_surcharge')>{{ __('messages.penalties.type_percentage_surcharge') }}</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount') }}" class="form-input w-full" min="0" step="1">
                ...
                <div>
                    ...
                    <input type="number" name="late_cancel_hours" id="late_cancel_hours" value="{{ old('late_cancel_hours', 4) }}" class="form-input w-full" min="1" max="72">
                ...
                <div>
                    ...
                    <input type="number" name="grace_count" id="grace_count" value="{{ old('grace_count', 0) }}" class="form-input w-full" min="0" max="10">
